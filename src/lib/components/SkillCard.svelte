<script lang="ts">
	...
	function openModal() {
		// ネイティブの showModal() を呼ぶと、自動で背景がロックされフォーカスが閉じ込められます
...
<button
	class={`group rounded-lg border-gray-300 bg-white/50 p-2.5 backdrop-blur-sm md:p-5 flex w-full cursor-pointer flex-col border text-left outline-0 transition-all ${skillColors.hoverBorder} dark:border-gray-700 dark:bg-black/20`}
	onclick={openModal}
>
	<div class="mb-4 gap-1 md:gap-4 flex items-center">
		<span
			class={`pr-2 text-2xl md:pr-0 md:text-4xl font-[NerdFont] transition-colors ${skillColors.hoverText}`}
			>{skill.logo}</span
		>
		<h3 class={`font-mono text-lg font-bold md:text-xl transition-colors ${skillColors.hoverText}`}>
			{skill.name}
		</h3>
	</div>

	<div class="mb-2 bg-gray-200 dark:bg-gray-800 w-full overflow-hidden rounded-full">
		<div
			class="h-2 ease-out rounded-full transition-all duration-1000 {progressColor}"
...
<dialog
	...
	class="max-w-md rounded-xl border-gray-300 bg-gray-100 text-darktext p-6 shadow-2xl backdrop:bg-black/60 backdrop:backdrop-blur-sm dark:border-gray-700 dark:text-lighttext m-auto w-full border transition-transform dark:bg-[#111111]"
>
	<button
		type="button"
		class="right-4 top-4 text-2xl text-gray-500 hover:text-black dark:hover:text-white absolute cursor-pointer font-[NerdFont] outline-0 transition-colors"
		onclick={closeModal}
	...
	<div class="mb-6 gap-4 flex items-center">
		...
		<div class="pl-2">
			<h3 id="skill-modal-title" class="mb-1 text-2xl font-bold">
				{skill.name}
			</h3>
			<span class="rounded px-2 py-1 text-xs font-semibold text-white inline-block {progressColor}">
				{currentLevel}
			</span>
		</div>
	</div>
